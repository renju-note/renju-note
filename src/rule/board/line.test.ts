...
test('BlackFives1', () => {
  ...
  expect(line.rows.get(true, 'five')).toMatchObject([{ start: 0 }])
})

test('BlackFives2', () => {
  const line = new Line({ size: 7, blacks: 0b0111110, whites: 0b1000001 })
  expect(line.rows.get(true, 'five')).toMatchObject([{ start: 1 }])
})

test('BlackFives3', () => {
  const line = new Line({ size: 8, blacks: 0b01111101, whites: 0b00000000 })
  expect(line.rows.get(true, 'five')).toMatchObject([{ start: 2 }])
})

test('BlackFives4', () => {
  const line = new Line({ size: 6, blacks: 0b111111, whites: 0b000000 })
  expect(line.rows.get(true, 'five')).toEqual([])
})
